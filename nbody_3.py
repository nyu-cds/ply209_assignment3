...
def update_vs(v1, v2, dt, dx, dy, dz, m1, m2):
	...
	v1[0] -= dx * cmp_b(m2, dt, dx, dy, dz)
	v1[1] -= dy * cmp_b(m2, dt, dx, dy, dz)
	v1[2] -= dz * cmp_b(m2, dt, dx, dy, dz)
	v2[0] += dx * cmp_b(m1, dt, dx, dy, dz)
	v2[1] += dy * cmp_b(m1, dt, dx, dy, dz)
	v2[2] += dz * cmp_b(m1, dt, dx, dy, dz)
	...
	for body1 in BODIES.keys():
		for body2 in BODIES.keys():
			if (body1 != body2) and not (body2 in seenit):
				...
				(dx, dy, dz) = cmp_del(x1, x2, y1, y2, z1, z2)
				upd_vs(v1, v2, dt, dx, dy, dz, m1, m2)
				seenit.append(body1)

	for body in BODIES.keys():
		(r, [vx, vy, vz], m) = BODIES[body]
		upd_rs(r, dt, vx, vy, vz)
